...
# (bob_dplay_create <- CoCreateInstance(CLSID_DirectPlay)), so if this passes and the game still
...
OBJ="$ROOT/build/SRC/compat/CMakeFiles/bob_compat.dir/bob_dplay.cpp.o"
if [ ! -x "$PROBE" ] || [ "$ROOT/tools/dplay_probe.cpp" -nt "$PROBE" ] || [ "$OBJ" -nt "$PROBE" ]; then
  echo "  building probe..."
  g++ -m32 -fno-pie -no-pie -w -DBOB_LINUX -DFF_LINUX -I "$ROOT/SRC/compat" -I "$ROOT/SRC/H" \
      "$ROOT/tools/dplay_probe.cpp" "$OBJ" -o "$PROBE" || { echo "  FAIL: probe did not build"; exit 2; }
fi
...
# 2. host + client
( "$PROBE" host >/tmp/dp_host.txt 2>&1 & )
sleep 2
"$PROBE" join >/tmp/dp_join.txt 2>&1; jrc=$?
sleep 3
grep -q "^PASS host" /tmp/dp_host.txt && say "host RECEIVED the client's packet" "yes" \
...
pkill -x dplay_probe 2>/dev/null

echo "----------------------------------------"
if [ "$fail" -eq 0 ]; then
  echo "PASS: discovery, join and a packet across two processes"
  grep -a "RECEIVED" /tmp/dp_host.txt | sed 's/^/  /'
  exit 0
fi
