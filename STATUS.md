...
deterministic, not a one-off, and it will keep eating player data until fixed. <br>**Next step:** find WHICH gate writes it — the campaign, soak and strategic gates all fly — and give it the same stash/restore the MA suite already does for the player's campaign save (`port/parity_2d.sh` PIN/`SAVEDIR` pattern). A gate that eats player data is a bug in the gate. |
