...
| item | what | state |
|---|---|---|
| **R20** | campaign flight: a flat dark square floats in the sky | NEW 2026-08-31, PO screenshot. Measured: ONE flat colour (40,52,52) = an untextured quad, near the ground's shade. MA PO-82's class, inverted. |
| **R21** | campaign screen: Messages list overflows its dialog by ~500 px; the map leaves a third of the screen black; a stray dialog fragment at the origin | NEW 2026-08-31, two PO screenshots. Three separate defects, measured. |

(Before R20, every row was closed and the full suite green end to end — `RUNS: all clean`,
`PLAYER DATA: untouched`, exit 0, verified 2026-08-31.)
...
| R20 | 🔴 **Campaign flight: a flat dark square floats in the sky** (PO 2026-08-31, screenshot) | **PO:** *"bob campaign, fly - there is a black square that floats around in the air. And when I pressed printscreen (may have hit a nearby button first) I get this."* Evidence: `doc/reference/po_2026-08-31_blacksquare/inflight_gfx_blacksquare.png` (1852x1047, in-flight with the GFX dialog raised over the 3D). <br>**MEASURED from the PO's own frame, before any hypothesis:** the square spans roughly **(650,35)-(830,140)** and contains **exactly ONE distinct colour**, RGB **(40,52,52)**. The sky around it is (179,198,205). So it is a **flat-shaded untextured quad**, not a texture that loaded wrong — a failed texture would show noise or a recognisable image, and a single flat colour is what an untextured draw in its vertex colour looks like. <br>⭐ **Its colour nearly matches the GROUND band** at the bottom of the same frame, (34,44,48) — 6 units apart per channel. That points at ground/terrain-shaded geometry drawn in the SKY, or an object taking the ground's shade. <br>⚠️ **This is MA PO-82's failure CLASS, inverted.** There an unresolved texture handle became `glDisable(GL_TEXTURE_2D)` and drew WHITE in the model's vertex colour; here the same mechanism would draw DARK if the vertex colour is dark. MA's instruments for it now exist and are worth cross-porting: a per-draw census that separates *textured* / *untextured-because-handle-0* / *untextured-because-lookup-failed*, since MA proved handle-0 draws are invisible to a resolver counter (they never reach it). <br>⚠️ **`GATE 4b` already asserts `blackTex=0` and PASSES**, so either this is not the terrain-tile path that gate covers, or the gate's scope misses it. **Do not read that green as covering this** — establish which before trusting it. <br>**Also visible in the same frame, recorded but NOT claimed as related:** (a) a grey ellipse with a dark aircraft silhouette floating in the sky at upper-left — a shadow blob at altitude?; (b) the settings read **`Item Shading = Reflections` while `Reflections = Off`**, which may be an inconsistent pair worth checking; (c) `Weather Detail = Off`, `Auto Frame Rate = Off`, `Lowest Frame Rate = 8`, `3D Resolution 1920x1080x16` against `Campaign Resolution 1920x1080x32` — a **16-bit 3D mode with a 32-bit campaign mode**, worth confirming is intended. <br>**First step is a reproduction with the census on, not a code read.** The square "floats around", so it moves with the world or the camera — which of those it tracks is the single most useful next measurement, and it decides between misplaced world geometry and a screen-space artefact. | <br>⭐ **S397 (2026-09-01) — the instrument for this ALREADY EXISTED; it was missing one field, and that field is the discriminator.** Done without the display. <br>I started cross-porting MA's PO-82 per-draw census and found `BOB_TRACE_GREY` already in `bob_video.cpp`, built for exactly this — its own comment says *"a square floating in the 3D view"* — and carrying two recorded mistakes worth keeping: it first filtered `!is2D` (a 10-minute real-GL flight then reported **0 untextured draws**, which was only evidence about the half it was looking at), and it first deduped on the surface pointer `t`, which is **nil for every texturing-disabled draw**, so they all collapsed to one entry and the first — a legitimate untextured loader screen — silently swallowed all the rest. **I deleted my duplicate rather than ship a second census.** <br>**What it lacked: the FLAT COLOUR.** An untextured draw paints in its vertex colour, and the PO's square was measured off his own frame as exactly one colour, **RGB (40,52,52)** against a sky of (179,198,205). Surface, prim, count and fvf are shared by thousands of legitimate untextured draws; **the colour is the one field that can pick his square out of them.** `[grey]` now prints it, read from the existing `FvfLayout` (D3DCOLOR is ARGB), and says *"none (uses glColor state)"* when there is no colour array rather than inventing `0x00000000`. <br>**Next, one display slot:** a campaign flight with `BOB_TRACE_GREY=1`, then find the `[grey]` line whose flat colour is ≈`R40 G52 B52`. Its `fvf`, `prim` and one-shot backtrace then name the draw — and the existing trace already prints a caller backtrace for the first hit.
| R21 | 🔴 **Campaign → Convoys → campaign screen: the Messages list overflows its dialog, and the map leaves a third of the screen BLACK** (PO 2026-08-31, two screenshots) | **PO:** *"campaign/convoys/campaign screen mess"*. Evidence: `doc/reference/po_2026-08-31_campaign_screen/` — `messages_overflow_zoomout.png` and `messages_overflow_black_map.png` (both 1852x1047). <br>**Three distinct defects in the same frames. Recorded separately because they are unlikely to share a cause.** <br>**(1) THE MESSAGES LIST IS NOT CLIPPED TO ITS DIALOG.** The `Messages` panel ends at about **y=428**; the rows keep drawing to **y≈946** and **y≈917** in the two shots — **~500 px and ~477 px of overflow**, straight over the map with no backing. Everything from the `06:50 Wittering AF` row down is outside the dialog. The rows inside the panel look correct, so this is a **clipping/extent** failure, not a layout or data failure: the listbox is drawing every item it holds instead of only those inside its client rect. <br>⭐ **This is MA PO-77's family** (*"list text bleeding across the thumbnails"*, a `CLoad` list over a panel). MA established there that the fix is per-listbox (`ma_lb_force_fill`) and that the z-order rule must not be re-broken; and `RLISTBXC.CPP:518`'s opaque backing fill is env-gated OFF by default (S70/S71, deliberately — gold shows through in the prefs tabs). **Do not re-enable that fill globally to fix this**; MA already found that path costs parity elsewhere. <br>**(2) THE MAP RENDERS ONLY THE LEFT ~64% OF ITS AREA.** In the zoomed shot the band **x 1190..1800 is black** (34% of sampled columns, full height). **The squadron icons ARE drawn there** — 169 green overlay pixels measured at x 1190..1238, y 196..320 — so **the overlay layer covers ground the terrain layer does not.** That mismatch is the whole clue: two layers, one extent each, disagreeing. The un-zoomed shot has no black band, so it is **zoom/extent dependent**. <br>**(3) A STRAY DIALOG FRAGMENT sits at the top-left corner** in both shots — a title bar with `? ✓ ✗` buttons at roughly (0,18)-(232,40), with `SECTOR W` text beneath it in the zoomed shot. A second dialog is being drawn at the origin with no body. <br>⚠️ **Do NOT assume (1) and (3) are the same bug** because both involve dialogs, and do not assume (2) is a map-data problem before checking the two layers' extents — an overlay drawing where terrain does not is a clipping/viewport disagreement, which is the same *shape* as (1) but in a different subsystem. <br>**Next: reproduce on the convoys campaign** (`tools/bob_convoy_campaign.sh` already drives Campaign → Convoys → directives → fly, and now traces per-control skips), raise the Messages dialog, and capture. Measure the listbox's client rect against the rows it emits — that single number decides (1). | <br>⭐ **S393 (2026-08-31) — DEFECT (1) IS LOCALISED, and the code already knows it happens.** Done without the display, which the PO's own session was using. <br>`bob_ole.cpp:436-470`: the listbox drawer lays out **`contentH()`** — the height of ALL rows — and compares it against the control's rect height `hpx`. When content overflows, an **`[extent] OVERFLOW`** line already reports it by name, with the rows that fall outside. So this class of defect is instrumented; nobody has pointed it at the Messages dialog. <br>**The mechanism, exactly:** S173 clips the control's **art blit** to its own rect via `bob_gdi_setdibits_clip` — but that clip is on the `SetDIBitsToDevice` path and **says nothing about row TEXT**, which the code states in as many words. So the backing art stops at the dialog edge while the row text keeps painting down the screen. That is precisely the PO's frames: a panel ending at y≈428 with rows continuing to y≈946, no backing behind them. <br>⚠️ **AND THE OVERFLOW IS CURRENTLY DELIBERATE, which the fix must reckon with.** S207 set `host->hitH = (ch > hpx) ? ch : hpx` — *widening the hit test to the content height* so that drawn-but-outside rows could be clicked, because MA's title menu had painted 199 px of rows in a 100 px listbox and the lower ones (Replay among them) answered no click. Its note is explicit: *"widening the per-control hit test cannot move a pixel — it only makes drawn rows answer."* **So the port chose to make the overflow usable rather than to stop it.** For a 5-row menu that is a reasonable patch; for a 40-row message log it paints half the screen. `BOB_NO_DRAWH=1` reverts the hit-test half. <br>**So the fix is not "add a clip" — it is to decide what a list that outgrows its box should DO** (scroll, or clip and page), and apply it without re-breaking the menus S207 fixed. Those want different answers, which is why this is a design decision rather than a one-line bound. <br>**Next, and it needs the display:** reach the campaign screen with `BOB_DUMP_MENU=1` and read the `[extent]`/`[menu]` lines for the Messages listbox — `rect`, `contentH`, `rows`. `contentH − hpx` is the overflow in pixels and should match the ~500 px measured from the PO's screenshot. If it does not, the row text is coming from somewhere other than this drawer. <br>⭐ **S395 (2026-09-01) — the missing clip now EXISTS and is switchable; still no display, so not yet judged.** S393 established the mechanism: `bob_gdi_setdibits_clip` bounds a control's ART BLIT but lives on the `SetDIBitsToDevice` path and never touched glyphs — confirmed by reading `bob_gdi_font.cpp`, whose glyph loop bounds to the framebuffer and nothing else. <br>**Added `bob_gdi_text_clip` / `bob_gdi_get_text_clip`** and one line in the glyph write loop. An empty rect means no clip, so the default path is byte-for-byte what it was. `bob_ole.cpp` sets it around the rows it paints, **behind `BOB_CLIP_ROWS=1`, default OFF.** <br>⚠️ **Default off on purpose, and this is the design tension S393 flagged.** S207 deliberately made overflow rows CLICKABLE — it widened the hit test to `contentH` because MA's title menu laid out 199 px of rows in a 100 px box and the lower ones (Replay among them) answered no click. **Clipping without deciding what an over-long list should DO would make those rows invisible as well as unreachable — trading a cosmetic defect for a functional one.** The PO's 40-row Messages log wants scrolling or paging; a 5-row menu wants a correctly sized box. Same symptom, different fixes. So the switch exists to measure the cosmetic half in isolation, **not to ship a fix.** <br>⚠️ **And it may turn out to be a no-op, which would itself be the finding.** The clip is set around the ART-BLIT window and restored after it, so it only bites if the row text is painted inside that same window. If `BOB_CLIP_ROWS=1` changes nothing on screen, the row text comes from a different path than the drawer, and S393's localisation is incomplete. **One display slot answers it:** run the campaign screen with and without the flag, plus `BOB_DUMP_MENU=1` for the `[extent]` line, and compare the pixels below the panel edge. <br>🎯 **S398 (2026-09-01) — ROOT CAUSE, and it is ONE cause for BOTH ports: the compat layer throws away `ETO_CLIPPED`.** <br>`CRListBoxCtrl` clips its rows correctly. `RLISTBXC.CPP` calls <br>`pdc->ExtTextOut(x, y, ETO_CLIPPED, cliprect, text, len, NULL)` <br>for **every row it paints** — the original game asks for clipped text. But `afxwin.h`'s <br>`BOOL ExtTextOutA(int x, int y, UINT, LPCRECT, LPCSTR s, UINT n, LPINT)` <br>took the options word and the clip rectangle as **UNNAMED parameters and discarded both**. The request was accepted and dropped, so every `ExtTextOut` in the port drew unclipped. <br>**That is the PO's Messages log painting ~500 px of rows past the dialog edge over the map — and MA's PO-77 replay-save list bleeding across the film-strip art. One cause, two ports.** <br>⚠️ **Same shape as S171 in this very function**, where `SetTextAlign` was ignored because the code `{ return 0; }`-ed the request and the map's scale ruler ran off the screen. **An unnamed parameter is a dropped requirement**, and this function has now had two. <br>**Fix:** honour `ETO_CLIPPED` using the text clip added in S395 — the rect takes the same viewport shift as x/y, and the previous clip is saved and restored so nesting is safe. `BOB_NO_ETOCLIP=1` reverts as the negative control. Builds clean. <br>**This supersedes S395's `BOB_CLIP_ROWS` as the fix candidate:** that switch clipped at the DRAWER, guessing at rows; this clips where the GAME asked for it, at the rect the game supplied. `BOB_CLIP_ROWS` stays as a measurement aid only. <br>⚠️ **Not yet seen on screen** — the PO's session has held the display for 2h17m. The A/B is now sharper than before: default vs `BOB_NO_ETOCLIP=1` on the campaign screen, expecting the rows below the panel edge to vanish in the default arm. <br>⚠️ **And the design tension from S393 remains real:** S207 widened the hit test so overflow rows could be CLICKED. Clipping them makes them invisible as well as unreachable, so a list that outgrows its box still needs to scroll or page. This fixes the bleed, not the overflow. <br>✅ **S402 (2026-09-01) — BoB's clip is MEASURED, and it works.** MA's twin probe (S401) found MA's identical-looking `ETO_CLIPPED` wiring was **inert**, so BoB's — written the same way, by reading — could not be trusted until measured. `tools/clip_probe.cpp` stubs `bob_gdi_dc_bits` with a plain buffer and links the SHIPPED font object: no SDL, no display, no gl-lock queue. <br>`unclipped: left=1071 right=1833` · `clipped to x<150: left=1071 **right=0**` · `empty rect: left=1071 right=1833` — **the clip clips, and an empty rect correctly does not.** <br>⭐ **And the asymmetry with MA is now explained rather than mysterious:** BoB has ONE glyph writer, and S395 guarded it. MA has TWO — `putpx` (bitmap-font fallback and background fill) and `blendpx` (the TTF writer that draws every real string) — and its clip was on the wrong one. Same apparent fix, opposite outcomes, for a reason that only a pixel test could surface. <br>`GATE CLIP` is in the suite and needs no display, so it runs even while the PO is playing.
| R22 | 🟢 **FIXED (S392): ten gates ended with `pkill -x bob` and would kill a session the PO was PLAYING** | Found the direct way, not by audit: the PO had bob open, the multiplayer UI sprint needed `bob_mp_uijoin.sh`, and that gate ends `pkill -x "$(basename "$BOB")"` — which kills **every** bob on the machine. Ten gates did it: convoy_campaign, combat_soak, mp_connect, mp_uijoin, dialslots, r9_layout, replay_record, r1_continuous, settings_nav, detect_probe. <br>**Same family as two hazards this port has already paid for:** R17/S366 and S373 (a gate reaching outside its own sandbox and overwriting the player's `replay.dat`), and the `pkill -f` self-match — **a process searching by NAME cannot tell its own target from someone else's.** <br>**Fix:** `tools/bob_safe_kill.sh`. `bob_snapshot_pids` records the PIDs that existed before the gate launched anything; `bob_kill_new` kills only PIDs that appeared after. If no snapshot was taken it **refuses to kill anything and says so**, because killing everything is the failure it exists to prevent. <br>**Verified in BOTH directions, with the PO's session live on pid 2341852:** with a stand-in binary named `bob` started after the snapshot, `pgrep -x bob` saw **both** PIDs; `bob_kill_new` killed **only** the new one (2348119) and the PO's session survived. A one-directional test would have proved nothing — "kills nothing" passes the protection half and breaks the cleanup half. |
| R19 | 🟢 **Cross-port from MA PO-88: bob does NOT have the texture-quality ratchet — it does not build that code at all** (S386, 2026-08-31) | MA's PO-88 is `direct_3d::SetTextureQuality` being unable to RAISE texture quality (the `&&`'s `++` and the `else if`'s `--` cancel). The engine is shared, so the obvious move is to grep bob — and **`SRC/HARDWARE/WIN3D.CPP:6668` carries the identical three lines.** <br>**It is dead code.** Three independent confirmations: it appears nowhere in `build/build.ninja`; nothing `#include`s it (`_HARD.CPP:78` has `//#include "..\HARDWARE\Win3d.cpp"`, commented out, naming a file that does not exist here); and **`nm -C build/bob` contains ZERO `direct_3d` symbols** against 15041 total. This port replaced the whole Direct3D wrapper with the SDL/GL compat layer. `CONFIG.CPP` and `SDETAIL.CPP` are unbuilt for the same reason. **So the cross-port answer is "not applicable", and a grep alone would have answered "bob has it too".** <br>⭐ **`tools/bob_dead_sources.sh`** now names every uncompiled source: **390 of 489**, of which **58 have a same-name COMPILED twin** — bob has ma's case-duplicate trap as well (`SRC/MFC/MIG.CPP` is dead; `SRC/MFC/MIG.cpp` is what `_MFC.CPP:82` includes). Authority is `build.ninja` plus the transitive `#include` closure, because these unity builds pull sources in by `#include`. <br>⚠️ **The checker was wrong twice before it was right, and both are worth keeping:** (1) it missed Windows-separator includes (`..\MFC\MIG.CPP`) and (2) I then "verified" it against `MIG.CPP` and called a correct answer a false positive — the uppercase file really is dead, and my assumption about which twin was live was the error. Validate a checker against files whose status you have *confirmed*, not assumed. | <br>⚠️ **S390 (2026-08-31) — the checker I shipped yesterday was wrong twice, and its headline number was over-reported.** Audited it by asking a question it should answer: *do any gates or docs cite a file that is not compiled?* <br>**Result on the axis that matters: NO GATE ACTS ON A DEAD FILE.** The three that name one (`bob_mp_uijoin.sh`/`COMMS.CPP`, `bob_gates.sh`/`WINMOVE.CPP`, `bob_r1_continuous.sh`/`3D.CPP`) do so in **comments only**, and the third is a false match on `STUB3D.CPP`. <br>**Two real bugs in the tool, both found by checking rather than trusting:** <br>1. **The twin test never checked the twin.** It counted any same-name file as a "live twin" and reported **58**. `RCOMBO.CPP`, `RLISTBOX.CPP` and `PERSONS2.CPP` have two or three copies of which **every one is dead** — the MFC control sources, all replaced by the compat layer. The true figure is **1** (`MIG.CPP` shadowing the compiled `MIG.cpp`). <br>2. **The `#include` closure extracted nothing.** `s/.*"//` is GREEDY, so on `#include "./MIGView.cpp"` it ate through the CLOSING quote and produced an empty string — meaning every source reachable only by `#include` was reported dead. Capturing between the quotes fixes it; `MIGView.cpp` and `Fullpane.cpp` are now correctly live. <br>**Corrected: 389 of 489 uncompiled, 1 with a compiled same-name twin.** Validated against six files whose status is known — the standard the checker itself demanded and that I had not applied to it. <br>**And the bigger trap is the one the wrong number hid:** most dead files are not built in ANY copy, so a doc naming `RLISTBOX.CPP` points at nothing live at all — the real listbox is `SRC/RLISTBOX/bob_ole.cpp`. That is worse than a case-shadow, because there is no correct file with that name to find.
| R18 | 🟢 **FIXED (2026-08-31): the game clock ran 1.9% slow — the periodic timer accumulated its own sleep overshoots** | Opened by R16-S5, which found the padlock extrapolation drifting ~20 ms/s and fixed that symptom by reading the wall clock. This is the CAUSE. `bob_threads.cpp`'s `timer_thread` slept the 40 ms period in 5 ms `usleep` slices and *then* ran the callback, so every period cost the period **plus** eight sleep-wakeup overshoots **plus** the callback's runtime, and the error accumulated. The timer drives the move cycle, and the move cycle advances `timeofday += FRAMETIME` — the game's own clock. **Measured: 40.757 ms against a nominal 40 = 1.89% slow, about 11 seconds lost per ten minutes of mission.** Fix: `clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME)` against a fixed next-deadline, so a late callback is absorbed by the next period instead of pushing the schedule back; if it falls more than one period behind (a stall) the deadline re-bases to now rather than firing a catch-up burst. **Achieved: 40.000 ms/period.** Gate `tools/bob_r18_timer.sh` (in the suite) runs both arms and requires the control to drift — `BOB_TIMER_SLICES=1` restores the old loop, `BOB_TRACE_TIMER=1` reports the achieved period. |
| R16 | 🟢 **BOTH FIXED — and the full suite is green with GATE R16 in it (S6: `PLAYER DATA: untouched`, `RUNS: all clean`, R16 arms: control drifts to 1000 ms / fix peaks at 44 ms, resets 0/8700)**  — band (vsync, S355) and JUDDER (S5: the padlock extrapolation clock was a counter, and it drifted)** | ⭐ **S5 (2026-08-30): ROOT-CAUSED AND FIXED, MEASURED BOTH WAYS.** `ViewPoint::move_time_ms` is the timestamp `view_dt` is measured from, and `view_dt` is the extrapolation `ViewFudge` applies to the padlocked item (`World += vel*view_dt`) so it is drawn where it will be between 25 Hz world moves. `STUB3D.CPP:2304` advanced it by a **nominal 40 ms per move cycle — a counter standing in for a clock**. The port's pthread move timer runs ~2% slow (accumulated 5 ms usleep slices overshoot), so the counter drifted ~20 ms/s against wall time; `view_dt` climbed to the 1000 ms clamp in ~50 s and snapped to 0. **Measured on a padlocked sortie: max=1000 ms, mean 466, 68% of frames extrapolated >300 ms.** At 100 m/s closing the bogie is drawn up to 100 m ahead of truth then yanked back — worse with speed, worse up close: the PO's report to the letter. **Fix: stamp `timeGetTime()` at each move instead of counting.** With it: **all 8700 frames <50 ms, max 45, mean 20, zero resets.** Gate `tools/bob_r16_viewdt.sh` (in the suite as GATE R16) runs both arms on a flight that provably padlocks and asserts the control drifts, the fix does not, and the tap fired. <br>Two wrong turns kept: (1) I first read the field as "stamped once, never again" — a grep filter dropped the `+=` line for its trailing comment; the unfiltered grep and the per-frame series refuted it (drift at 1/50 wall rate, not a 1 Hz sawtooth). (2) A stamp placed BESIDE the counter fought it — 1600 negative-delta resets, extrapolation disabled 41% of frames — so it had to REPLACE it. <br>⚠️ **Opens R18:** the same 2%-slow timer advances `timeofday += FRAMETIME` per tick, so the game clock itself runs slow; the timer should use absolute deadlines (`clock_nanosleep TIMER_ABSTIME`), not accumulated slices. <br>**History (S355–S357):** **PO 2026-08-30**, verbatim: *"fix 3D screen flicker. Sometimes there is a flickering band at the top of the screen, sometimes when you get close to a padlocked bogie it feels like the frame rate is about 5 fps at a time of rapid motion. On an i7 PC with a 6GB nvidia card there should never be a low fps kind of effect."* <br>**TWO SYMPTOMS, and they may be two bugs — do not assume one cause.** (a) a flickering BAND at the TOP of the screen; (b) a judder that reads as ~5 fps specifically *near a padlocked bogie during rapid motion*. <br>**The PO's hardware point is the requirement**: an i7 with a 6 GB nvidia card should never produce a low-fps effect in this game, so if the frame rate really is dropping the cause is the port, not the machine. <br>⚠️ **FIRST ESTABLISH WHICH IT IS.** "Feels like 5 fps" has at least three causes that need different fixes and look identical: the frame rate genuinely collapsing; the frame rate holding while the CAMERA moves in large steps (a padlock view that updates at a lower rate than it draws); or frames being presented out of order/duplicated. **Measure the actual frame interval before touching anything** — BoB already has `[lifetime]`/frame counters and `BOB_SHOT2D_EVERY`, and a per-frame timestamp histogram would separate (1) from (2) immediately. <br>**The band at the top smells like a present/tear issue** (vsync, a partial blit, or the 2D overlay strip drawn at a different cadence from the 3D) — `R9`'s work on present-time offsets and `g_uiOff` is adjacent and worth reading first. <br>⭐ **S355 — `BOB_TRACE_FRAMETIME` built, and it answers the first question.** Wall-clock interval between consecutive presents, as a histogram plus the worst offenders. Over **24,600 frames** of automated flight: <br>⚠️ **S388 (2026-08-31) — R18 KILLED THIS GATE'S NEGATIVE CONTROL, and the gate said so instead of going green.** A full-suite run went red here: `FAIL/RETHINK: the control did NOT sawtooth (max=42 resets=5) -- the premise is wrong`. <br>**Why:** R16's control is the legacy frame counter (`BOB_NO_MOVESTAMP=1`), which only sawtooths when the move timer itself drifts. **R18 then gave that timer an absolute schedule**, so `FRAMETIME * 10` now tracks real time and the control stopped misbehaving — max 42 ms where it used to reach 1000. A negative control that no longer reproduces the defect proves nothing about the fix, and this gate refusing to pass on one is exactly the behaviour wanted. <br>**Fix:** the control now reverts **both** fixes (`BOB_NO_MOVESTAMP=1 BOB_TIMER_SLICES=1`), i.e. the code as it stood when the PO reported the judder. Re-measured: **control max 1000 ms / 11 snaps, fix max 41 ms / 0 resets in 8700 frames**, PASS. <br>**General lesson, and it has now bitten twice in one day** (julia's `JM_ELASTIC_WALL` was a control arm that had silently become identical to the treatment): **when a later fix removes the conditions an earlier gate's control depends on, that control quietly stops being one.** Controls need re-validating whenever something upstream of them changes.
