...
**Practical consequence:** the trigger for taking `gl-lock` is *load*, not *pixels*. You have
been wrapping your `SDL_VIDEODRIVER=dummy` `BOB_SHOT`/`BOB_RUN_INIT` captures in `gl-lock` even
though they never reach the screen — **keep doing that.** It looks redundant under the old
pixel rationale, but those runs are CPU-heavy software rendering, and on 4 cores they can starve
MiG Alley's stress gate into a false `HANG` that reads as a port defect in *its* repo. Note
`~/bin/gl-lock`'s own header comment still states the old rationale; `~/CONCURRENCY.md` and this
file are the authority.


## This repo's slot

`bob` runs concurrently with the other two flight sims — separate repos, no shared files.
