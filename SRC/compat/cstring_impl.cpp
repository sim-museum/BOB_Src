...
/* ---- formatting ----------------------------------------------------------- */
/* Linux/GCC porting fix for the MFC "CString in printf varargs" idiom.
 * The game pervasively writes CSprintf("%s",aCString) / str.Format("%s",aCString) WITHOUT a
 * (LPCTSTR) cast. On MSVC a CString is passed to varargs BY VALUE (it is one pointer member, so
 * %s reads the char*). Under the Itanium/SysV C++ ABI (Linux GCC) a class with a non-trivial copy
 * ctor/dtor -- CString -- is passed to varargs BY INVISIBLE REFERENCE (a pointer to the object),
 * so a plain vsnprintf %s prints the object's m_pchData *bytes* as text -> garbage. (Confirmed
 * empirically; e.g. the controls-config device/axis combos were unreadable.)
 *
 * Fix: pure-numeric formats keep the trusted libc vsnprintf path (zero change). Only when the
 * format contains %s do we walk the conversions and, for each %s, decide whether the argument is a
 * CString-by-reference (deref to its data pointer) or a genuine char* (use as-is) by validating the
 * CStringData{nRefs,nDataLength,nAllocLength} header that sits just before a real CString's buffer,
