...
#define	F_COMMON
	...
	#include	<stdio.h>
#if BOB_LINUX
#include <stdlib.h>   /* getenv, S315 guard */
#endif
//---------------------------------------------------------------------------
...
#if BOB_LINUX
	/* R4.5 (S37): honor this function's OWN contract (the assert below) on Linux. A UID outside
	   [1, IllegalSepID] is illegal -- on Windows debug the assert catches it; on Linux the assert
	   doesn't halt, so a garbage UID (from an incompletely-restored deserialised reference, e.g. a
	   stale SquadTarget/targetindex post-load) indexes pItem[] out of bounds -> SEGV. Return the
	   SAME null reference the function already returns for UID==0, so the OOB read becomes the
	   handled-NULL callers already expect for 0 (they take &ConvertPtrUID(...) or NULL-check).
	   This is the compat-non-halting-assert class (cf. R1.3b Reg3dConv, 4.3c SetIndex): the game's
	   bounds-guard exists but INT3/assert doesn't fire on compat, so the OOB it guarded happens.
	   NOT a fake-valid sentinel (S29 caution) -- it returns NULL exactly where the game declares the
	   UID invalid. The underlying deserialise gap (why the UID is garbage) is the per-reference
	   FixupAircraft/SetTargetUIDs work tracked separately; this stops the OOB crash for the family. */
	if (!(tmpUID>0 && tmpUID<=IllegalSepID))
		return *(ItemBase*)NULL;
	/* S315: and the same for a NULL pItem, which S37 did not cover. The bounds guard above
	   validates the INDEX; it says nothing about the ARRAY. Opening the strategic map before the
	   world is built leaves pItem NULL, and CMapDlg::InvalidateAnotherItem -- driven from
	   CMIGApp::OnIdle, so it runs on its own without anyone asking -- walks the SAG band from
	   SagBAND calling this directly. Reproduced three times, identical every time:

	       CRASH: signal 11 fault_addr=0x4800  eax=edi=ebp=0x1200  ebx=0
	       Persons2::ConvertPtrUID  <- CMapDlg::InvalidateAnotherItem  <- CMIGApp::OnIdle

	   0x1200 IS SagBAND, so it dies on the loop's FIRST item, and 0x1200*4 == 0x4800 is exactly
	   pItem[4608] off a null base. Not the S37 family: the UID is perfectly valid, the array is
	   absent.

	   That NULL pItem is a REAL state and the game says so itself -- SlowConvertPtrUID, the very
	   next function in this file, exists to do `if (pItem != NULL)` before calling here. So the
	   engine already treats an absent pItem as legitimate; this path just never defended against
	   it, presumably because on Windows the map was never entered in that state. Return the same
	   null reference the function already returns for UID==0, which every caller handles (they
	   take &ConvertPtrUID(...) or NULL-check) -- InvalidateAnotherItem's own `if (p)` is right
	   there. BOB_NO_PITEM_GUARD=1 restores the crash for A/B. */
	if (pItem == NULL && !getenv("BOB_NO_PITEM_GUARD"))
		return *(ItemBase*)NULL;
#endif
...
UniqueID	lastsupergroup=currsupergroup;
	...
	while (loopcount--)
	{
		...
		switch (*(bfieldptr++))
		{
			...
			break;
	 		case T_inform:	
				{ EventVal _ev=constructvalue(bfieldptr); numinform=make_itemgroupcount(_ev); }	/* temp->non-const-ref */
			break;
...
//////more general test:|| *bfieldptr==T_x �� *bfieldptr==T_y �� *bfieldptr==T_z
					)	)
				||	(	W->rotate.complex
