...
// S59: these were dword-granular -- they cast p to ULong* and read/write a[bit>>5]
// (a 4-byte access). MakeField<...>'s `dataspace[BYTES]` is sized to the field width
// (e.g. 2 bytes for a 16-bit field), so a 4-byte access overruns it -> ASan
// global-buffer-overflow (e.g. MakeField<QFD,0,15>::operator|= -> BITSET on the
// 2-byte `nonplayer`, CSQuick1::CSQuick1, Squick1.cpp:141). On x86 (little-endian)
// byte-granular addressing targets the IDENTICAL physical bit (byte[bit>>3] bit
// (bit&7) == the bit the dword form set), so this is behaviour-identical AND never
// reads past the field. The *I (value) variants are untouched -- they don't deref p.
