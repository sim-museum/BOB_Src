...
## Capture-vs-gold STATE audit (SP.17, S149)

A verdict can be right about the port and still not be *earned*, if the native capture and the gold
shot are in different game states. Three rows were found in that condition — **#16** (S141), **#19**
(S148) and **#17** (S149) — and all three were caught *incidentally*, which is what prompted a
deliberate pass over all 19.

**What the pass found.** The three campaign rows shared one root: the drive recipes never selected a
campaign **phase**, so every capture sat in the Convoys default while all three gold shots are
**Eagle Attack, 12–23 August**. Fixed by adding the S141 phase token (`#1000:1`) to each recipe;
all three now match gold on state, and #17's "phase/date differ (state)" deviation is retired
outright rather than explained.

**The uncomfortable part is how they read before the fix.** None of the three was invisible — each
row *said* the state differed. #17's said "phase/date strings differ only by selected phase
(state)"; #19's said "fresh Convoys day … gold is 12 August Eagle Attack". They were **recorded as
deviations and then reasoned past**, because "that's just state" sounds like a reason to stop
looking. It is not: a state difference the recipe can fix is a defect in the *test*, and one it
cannot fix is a finding about the *port*. The two need separating, and this doc had merged them.

**The fourth case: S150 measured it, drew the wrong conclusion from it, and S151 retracted that.
Both halves are worth reading.** Four rows (#6, #9, #10, #11) cite "combo values (settings state)".

S150 instrumented the load and got `settings.cfg exists=1 … successfulLoad=0` — **the port finds the
file and fails to parse it**, falling back to factory defaults. That much is solid, and S151 fixed
it (the guard is a `__DATE__` stamp; see PORT.md S151).

**S150 then concluded "gold parsed the same file under Wine, so the deviation is ours". That was
wrong.** S151 checked the two things S150 had assumed:

- **Provenance.** The trace prints the stamp: `Rowan Savegame: Jul 19 2026`. A 2000-era BDG binary
  cannot stamp 2026 — **that file was written by our own port**, the day this install was made. The
  gold shots are dated **2026-06-24**, three weeks earlier, so gold never saw it.
- **Effect.** With the fix in, the four screens render **byte-identical** to the factory-default
  captures — the file holds default values anyway, so loading it changes no pixel.

**S152 then answered it from the pixels, and the ORIGINAL annotation was right — it just had no
evidence behind it.** Reading the actual differing values on #11 (Sim-Game) and checking each
against the game's coded factory defaults in `SaveData::InitPreferences`:

| row | gold | native | coded factory default |
|---|---|---|---|
| Weapons | Unlimited | **Realistic** | Realistic (`GD_UNLIMITEDARM` never set) |
| Ground Collisions | Off | **On** | On (`gamedifficulty \|= GD_GROUNDCOLLISIONS`) |
| Complex A.I. Pilots | On | **Off** | Off (flag untouched by InitPreferences) |
| Target Size | Low | **Medium** | `targetsize = TS_MEDIUM` |

**Our value is the coded default in every case; gold's is not in any of them** — and gold's set is
coherent as a player's deliberate configuration (unlimited weapons, ground collisions off, complex
AI on, small targets). So these differences are **genuine user-settings state in the gold capture**,
out of scope for a port verdict. **SP.20 retired — verified rather than assumed.**

The arc is worth keeping as a caution: S149 flagged this as *"plausibly benign, not verified"*; S150
declared it a port defect on an unchecked inference; S151 retracted that; S152 confirmed the
original note with evidence. **The first annotation was correct and the confident correction was
not** — which is an argument for marking things *unverified* and leaving them alone until measured,
rather than for trusting whichever claim is newest.

**The lesson, recorded because it is precisely what this section exists to catch:** a measurement
licenses only the claim it measures. `successfulLoad=0` proves *we* do not load the file; it says
nothing about what gold did. That step was an inference, and it was written up as a conclusion
without checking the file's date or whether loading changed a single pixel.

**Rows checked and clean on state:** #1 (title), #2 (QS default page), #3 (briefing via its own
scaffold), #5 (cockpit — first-flight state, deviations already named), #7, #8, #12, #13, #14/#15
(side-select, no campaign state), #18 (already carries the phase token since S141). #4 is the
by-design GAP.
...
| 6 | 55-25 | PC Config — More GFX (`options3d2`) | `BOB_CONFIGSCREEN=gfx2 BOB_SHOT=70` | `native-config-moregfx-2026-07-26.png` (sbs: `sbs-config-moregfx.jpg`) | **CLOSE (S124: label-for-label gold)** — all 12 rows labeled exactly as gold: Filtering/Smoke Effects/Texture Quality/**Town and forest raises**/Routes/A-C Shadows/Item Shadows/Horizon Distance/**Detail Level**/G Effects/Injury Effects/White Outs (BDG captions via PE DLGINIT + IDS string-table resolution). Deviations: combo values — **VERIFIED user-settings state (S152): ours are the game's coded factory defaults in every case, gold's are a player's chosen configuration. Out of scope for a port verdict**, one stray box artifact at G Effects, tab-row spread, font face. |
...
| 9 | 55-59 | PC Config — Sound (`soundoptions`) | `BOB_CONFIGSCREEN=sound BOB_SHOT=70` | `native-config-sound-2026-07-26.png` | **CLOSE (S124: structurally 1:1 with gold)** — Sound Driver header + wide driver combo + all 7 labeled rows in gold order (3d SFX Volume/SFX Processing/UI SFX Vol/Ambient SFX Vol/Radio Chatter Volume/Engine Volume/SFX Quality). The overlapped top label + stray top/bottom combos are GONE (template filter: BDG dropped the source's music combos). Deviations: combo values — **VERIFIED user-settings state (S152): ours are the game's coded factory defaults in every case, gold's are a player's chosen configuration. Out of scope for a port verdict**, "Default" vs gold "Primary Sound Driver" (faithful port string), font. |
| 10 | 56-15 | Sim Config — Flight (`flightoptions`) | `BOB_CONFIGSCREEN=flight BOB_SHOT=70` (`sim` also works) | `native-sim-flight-2026-07-26.png` | **CLOSE (S124: row-for-row gold)** — FLIGHT OPTIONS header row + all 10 rows labeled incl. the previously-missing Engine Management / Prop Pitch Control and gold's **109 Fuel Capacity** (BDG). Deviations: combo values — **VERIFIED user-settings state (S152): ours are the game's coded factory defaults in every case, gold's are a player's chosen configuration. Out of scope for a port verdict**, font. |
| 11 | 56-21 | Sim Config — Game (`gameoptions`) | `BOB_CONFIGSCREEN=game BOB_SHOT=70` | `native-sim-game-2026-07-26.png` | **CLOSE (S124)** — all 10 rows labeled+paired (Weapons/Vulnerable To Fire/Ground Collisions/Midair Collisions/Complex A.I. Pilots/Accel Off/Target Size/Auto Canopy/Text Info/**Aircraft Names**). Deviations: combo values — **VERIFIED user-settings state (S152): ours are the game's coded factory defaults in every case, gold's are a player's chosen configuration. Out of scope for a port verdict**, font. |
...
| 17 | 57-55 | Campaign enter-name (`campaignentername`) | `BOB_AUTOCLICK=1,1,#1000:1,1 BOB_SHOT=640` (S149 — the `1,1,1` recipe never selected a phase, so every capture was Convoys against an **Eagle Attack** gold) | `native-campaign-entername-eagle-2026-08-08.png` | **CLOSE (S126: date/labels in gold's large gold face + authored colors from the persisted stream; S125: REdit hosted + gold line layout)** — the name edit is the genuine hosted `CREditCtrl` (S125 REdit host: caption/word-list machinery live, blocking-keys + `CommsPlayerName` reach it), and the line layout now matches gold structurally: "Commander Bob" adjacent (IDC_ROLE right-aligned per its persisted alignment byte), "Luftwaffe <phase>" adjacent (IDC_SIDE right / IDC_PERIOD left), date centred. **S149 (SP.17 audit): that difference is GONE and it was never a port deviation.** The row used to say "phase/date strings differ only by selected phase (state)" — noticed, and dismissed as benign. It was a defect in the *recipe*: `BOB_AUTOCLICK=1,1,1` walks title → Luftwaffe → Begin without ever choosing a phase, so the port sat in the Convoys default while gold shows **Eagle Attack**. Adding the S141 phase-select token (`#1000:1`) makes the capture read **"Commander Bob / Luftwaffe Eagle Attack / 12th August - 23rd August"** — matching gold line for line (banner: `phase=1 autoclick=4/4`). Third verdict in this epic found resting on a capture-vs-gold state mismatch (after #16 and #19). Deviations: caret line not drawn in the capture (flash-timer state), font face (R6.2), typed-input not yet harness-verified headlessly (no key-injection harness; deferred with the caret check). |
...
(a) **the stray "Sweeps" spinner row is GONE as of S150** — and it was not a
visibility gap at all. A per-control dump (`BOB_TRACE_DIR=<dlg>`) put the six real target rows at
y=317..447 with 12 controls each and an **extra 11-control row at y=229** whose ids are the `...7`
members of the same families — i.e. index 7 of `FillTargetLists`' `for (i=1;i<8;i++)`, which is
**SWEEPSNDECOYS**. The game marks that branch dead (`INT3; //This should not happen. Patrols
removed.`, LWDIRECT.CPP:1626): **the fighter-sweep feature was cut, its logic is unreachable, but
its controls stayed in the BDG template**, so our DDX/template hosting drew them — over the
"Ground Attack Gruppen"/"Escort Gruppen" headers. Suppressed by id (`BOB_NO_SWEEPROW_SKIP` reverts);
the headers now read cleanly and gold agrees, because the shipped game has no sweep line.
(S146's `IDC_FIGHTERSWEEP*` guess named the wrong controls entirely — these are the measured ids.)
**Residual, smaller and distinct:** the "Sweeps" *label* static and the **"Escort 1:1"** row
(`IDC_ESCORT_PROPORTION`, id 1680, at y=195) still draw where gold shows neither. Note 1680 is NOT
dead code — `LWDirectives` drives it (`SetIndex(escortproportion/.19)`) — so it is a different
question from the sweep row and must not be assumed to have the same answer (**SP.21**); (b) the Escort tick-boxes draw as plain circles
