...
- **⭐ Delivered, and bigger than the story: `bob_evt_fire` matches `type_info` EXACTLY.** No
  ...
  general fix booked as **SP.12**, shared as **§8y**.
...
- **#19's remaining conditions, now exact:** (a) **arm the capture from the drive** (SP.7) — an
  ...
  how this drifted; (b) the raid path needs `dr->dirresults[0].targets[0]`, and index 0 is the RECON
  slot that `FillTargetLists` never fills (loop starts at `i=1`), so with Reconn Missions = 0 — gold's
  own value — DirectiveResults never opens and `MakeLWPackages` never runs. Route is a recon
  allocation or the `germanisauto` → `AutoLWPackages` branch. SP.11 carries with both named.
...
|---|---|---|---|---|
| **144** | ~8 | 6 | ⚠️ **PARTIAL — the title-bar OK works for the FIRST TIME (every base-registered event had been dead); map capturable on an active day, but #19 still not a valid oracle** | **(2026-08-08)** ⭐ `bob_evt_fire` matches `type_info` **exactly** (bob_eventsink.cpp:39) with no base-class walk, and all call sites pass the derived type — so **any `ON_EVENT` registered on a BASE class can never fire**, including `ON_EVENT(RDialog, IDJ_TITLE, 3=OK, OnOK)` + Cancel/Help, i.e. the engine's **title-bar ✓/✕/? buttons on every dialog in the game**. Invisible for the port's whole life because every previously-wired event was registered on the same class that received it. Worked around by firing under `typeid(RDialog)` (derived override still runs — `RDialog::OnOK` implicitly overrides compat's virtual `CDialog::OnOK`); general fix = **SP.12**; shared **§8y** (MA: this is likely why Player Log `?`/`✓` do nothing). **SP.11:** the faithful exit from S143's cancel-loop now works (`BOB_MAP_ACCEPTDIR` → `open-index=-1`), and with S143's suppression gave **the first clear strategic-map capture on an active campaign day**. **But it is NOT a valid #19 oracle and the state banner is how we know:** `phase=0 date=1250121600` — drifted to **15 Aug at x20**, 3 days past what a verdict would have claimed, phase reset, no unit icons. Remaining conditions now exact: arm the capture **from the drive** (SP.7), and satisfy the raid path — `dirresults[0]` is the RECON slot that `FillTargetLists` never fills (loop starts at i=1), so with Reconn Missions=0 (gold's own value) DirectiveResults never opens and `MakeLWPackages` never runs; route is a recon allocation or `germanisauto`→`AutoLWPackages`. **SP.8 investigated, not started** (hence 6/8): the extra row is the fighter-**Sweeps** row, **dead code** — `INT3; //Patrols removed.` SP.10 leak reproduced twice (184→368→1656). **Gates:** sweep 14/14 exit 0 + **14/14 BYTE-IDENTICAL vs pre-S142** (spans S142+S143+S144); safe default exit 0; dummy==GL byte-identical; flight 98.7% non-black. |
