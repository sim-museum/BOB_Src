...
| R4.5 | **Campaign uninit-state grind** — ◐ **POST-LOAD FORMATION CRASH FIXED (2026-06-25, S36); next layer = stale SquadTarget UID.** S36 fixed the S35 crash: `FixupAircraft` (`MAPCODE.CPP`) now resets the loaded SAGs' `fly.leadflight`/`nextflight`/`expandedsag` to NULL (they're `//save` raw pointers → stale; the SAG AI re-links at runtime — S29 invariant-restore pattern). ASan-confirmed `CountFormationSize` SEGV gone; sim advances to the next crash: `SAGExecuteWaypoint (SAGMOVE.CPP:1837) → ConvertPtrUID(SquadTarget(s))` garbage package-target UID in the `AM_LWPACKS` branch (S15–17 SAG-target/waypoint family, now post-load). Bare `./bob` 0; load-path only. _(S35: built `BOB_POSTLOAD_FF` ASan repro + root-caused the formation-pointer crash, correcting the prior GetCruiseAt characterization. Earlier: campaign sim runs + day advances 90s 2026-06-22.)_ | 8 | ◐ |
