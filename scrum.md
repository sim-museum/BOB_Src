...
| 7 | — | (in progress) | ⏳ env-var-free | **"No env vars" packaging (2026-06-17).** inc1: mission setup is game-driven (`SetQuickState`+`BoBFrag`), preflight mission-forcing retired for click mode. inc2: **a real menu click-through flies with NO `BOB_STARTFLYING`** — boot device-init (IntroSmackInit-equivalent) + always-on Launch3d bridge + frame-based auto-quit (fixed a black-flight bug: the old timer counted pump calls @~333/s → exited in ~1s before terrain loaded). Env-free flight renders the cockpit (91.1%); harness + R2.4 chain unregressed. **Remaining:** default-boot to the front-end (retire `BOB_FRONTEND`/`BOB_OLE_DRAW`) → mouse+keyboard only. |
