...
| 7 | — | ✅ **DoD MET** | **"No env vars" — DEFINITION OF DONE (2026-06-17).** inc1: mission setup game-driven (`SetQuickState`+`BoBFrag`). inc2: real click-through flies with NO `BOB_STARTFLYING` (boot device-init + always-on Launch3d bridge + frame-based auto-quit, fixing a black-flight timing bug). inc3: **bare `./bob` from the install dir boots the real title screen with ZERO env vars** (derives `BOB_DRIVE_C` from cwd, defaults front-end on, auto-runs); a mouse+keyboard playthrough flies a full mission (cockpit 91.1%) → debrief → menu. **The game boots + plays like the original** (minus env-blocked music + out-of-scope MP). Safe fallback (repo-dir bare `./bob` exits 0) + `BOB_NO_RUN`/`BOB_RUN_INIT` preserved. |
| 8+ | ~31 | — | — | R3 polish/peripherals (joystick via `DI_EnumDevices`, save/load round-trip, intro Smacker, front-end blit subsystem/DPI, deferred render fidelity: mirror UVs, trilinear mips, terrain combiner). |
