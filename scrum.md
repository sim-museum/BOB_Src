...
| R4.5 | **Campaign uninit-state grind** — ◐ **POST-LOAD CRASH ROOT-CAUSED (2026-06-25, S35).** Built the post-load ASan repro (`BOB_POSTLOAD_FF` lets `BOB_MAP_TIMER` fast-forward the loaded+`LaunchMap`-rebuilt world). First fatal crash is **NOT** `GetCruiseAt`/`Plane_Type_Translate` (that's the post-*mission* path) but a **SEGV in the radio-chatter intel path**: `MoveAllSAGs→RAFDetectLW::SetDetectionLevel→IntelBuffer::AddMessage→...→ArtInt::CountFormationSize (MSGAI.CPP:2367)` walking a garbage `trg->fly.leadflight`. Root: `flight_ctl`'s `leadflight`/`nextflight`/`expandedsag` (raw `AirStrucPtr`) are `//save`-serialized → a loaded raid aircraft carries a stale pointer from the saved session (S29 `RecostRaidList` class, one layer deeper). Candidate fix (deferred — deep, risks save/AI regression): NULL the loaded AirStrucs' formation pointers at the load boundary so the SAG AI re-links them. Banked precise root cause + reusable repro; bare `./bob` 0. _(earlier: campaign sim runs + day advances 90s 2026-06-22; SAG/waypoint NULL chain pinned.)_ | 8 | ◐ |
