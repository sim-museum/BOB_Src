...
| ID | Story | Pts | Status |
|---|---|---|---|
| R3.1 | **Scene lighting** — lit geometry (terrain/aircraft/cockpit/instruments) is under-lit while the sky is correctly bright (first-pilot report); the rear-view mirror shows the terrain bright, so the **main-view sun/ambient calc** is the culprit. Match the reference (instruments readable). | 8 | ☐ |
| R3.2 | **Cloud depth / draw-order** — fluffy-cloud billboards paint over the cockpit. Root: no shared per-pixel depth (terrain is a composited FBO layer). Give the 3D world a shared depth buffer **or** reorder the cloud draw so the cockpit occludes them. (BOB_ZTEST spike showed depth alone isn't enough.) | 8 | ☐ |
| R3.3 | **Cloud sprite fidelity** — checkerboard/opaque-white cloud quads → soft alpha-blended clouds (alpha-test/blend + texture filtering). | 3 | ☐ |
...
| **8–19** | — | — | — | **See §7a forward roadmap** → R3 faithful flight (lighting, clouds, mips, effects), R4 campaign, R5 control & sim, R6 front-end & media, R7 multiplayer. ~9–11 sprints to *all functionality*. |
