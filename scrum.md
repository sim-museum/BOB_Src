...
| ID | Story | Pts | Status |
|---|---|---|---|
| R5.1 | **Joystick** — ◐ **IMPLEMENTED (2026-06-22, PO connected a stick).** Full DirectInput→SDL_Joystick path in compat (enumerate, GetCaps, EnumObjects, SetDataFormat offset-learning, GetDeviceState reading real SDL axes/buttons/POV, QueryInterface). Keystone fix: distinct non-zero device GUIDs (BOBGUID made them all-zero → CreateDevice(joystick) returned the keyboard). Default flight mapping injected into runtimedevices (axis 0=aileron/1=elevator/2=rudder/3=throttle; `BOB_NOJOYDEFAULT` off). Verified the chain reads live axis values (`BOB_TRACE_JOY`); keyboard flight unregressed. `58bdd4a`. **Pending PO physical fly-test** (can't move the stick headlessly). | 5 | ◐ |
