...
| 7+ | ~31 | — | — | "No env vars" packaging (drive the loop from real mouse clicks + the player's own Alt+X; retire `BOB_STARTFLYING`/`BOB_AUTOQUIT`/`BOB_REFLY`) — the final DoD step. Then R3 polish/peripherals (joystick, save/load, Smacker, blit subsystem, render fidelity). |
