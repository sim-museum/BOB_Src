...
- **#19's remaining conditions, now exact:** (a) **arm the capture from the drive** (SP.7) — an
  ...
  fixed RECON slot, and `FillTargetLists` writes no `dirresults` at all. Since the grid does show
  allocations, the live hypothesis is that **`RefreshMissions` never ran headlessly**, to be settled
  by a gated trace, not by more reading.**]** SP.11 carries.
...
- **❌ Not delivered (S145.2) — decisive negative, not a stall.** `BOB_TRACE_DIR` in
  ...
  **S144's workaround had been running the wrong handler.** Structural cause: the logged child is an
  ...
  like success**.
- **Two corrections issued, both to S144, both mine:** (1) **§8y in the shared doc** claimed firing
