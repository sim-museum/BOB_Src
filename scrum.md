...
| 8 | 19 | 13 | ⏳ active | **Faithful flight I (2026-06-17).** ☑ R3.1 scene lighting (pilot-confirmed — was a pre-dawn time bug), ☑ R3.3 cloud checkerboard (smooth-alpha → GL_LINEAR). ◐ R3.2 clouds-over-cockpit re-scoped to a depth-architecture spike (FBO terrain has no shared per-pixel depth; BOB_ZTEST insufficient). |
