...
| R3.3 | **Cloud sprite fidelity** — fluffy-cloud billboards (CLOUD1–4 shapes, Lib3D `transparentPolys`) render as hard white-checkerboard quads, not soft clouds. Investigation: the keyed/transparent texels aren't discarded/blended on the cloud transparent path (the cross-port masked-texture-transparency pattern). Fix the alpha-test/blend + filtering for the transparent shape path. **Needs focused work** (not real-time). | 5 | ☐ |
