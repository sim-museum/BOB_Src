...
| R4.3 | **Campaign mission flow** — ◐ **MISSION BRIEFING REACHED (2026-06-24).** With the sim running (R4.5), drove the campaign's own map→mission seam: a `BOB_CAMPAIGN_FLY` scaffold performs the game's **interception** action (`HostilesList::OnClickedRbuttonauthorise` equivalent) — find an enemy raid whose squadron has spawned (`Squad::instance!=0`; a 0 instance SIGSEGVs `NewPackage`), `Todays_Packages_NewPackage(inst, RAF intercept profile)` **scrambles a real RAF interceptor**, then `LaunchFullPane(&bobfrag, UIR_FRAG)`. Key finding: a fresh day has only the **AI opponent's** packages (RAF sees LW raids, LW sees RAF patrols) — the player's package must be created. **Verified:** intercept → `NewPackage→packnum` → **bobfrag briefing renders** (montage backdrop + Back/Sim Config/**Fly** bar), no crash. Bypasses the CRToolBar/mission-folder OCX subsystem with the faithful public entry. Remaining: briefing hosted-control population (R6.3 class), **Fly→StartFlying** into campaign flight (reuses the QM Launch3d bridge), debrief→next-day. `bob_gdi_dump_to` one-shot capture tool added. _(earlier: SPIKED — clock wired, MoveAllSAGs crash, fixed in R4.5.)_ | 13 | ◐ |
