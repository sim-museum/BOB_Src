...
| 6 | 8 | 8 | ✅ **R2.4 shipped** | **Campaign continuity (2026-06-17).** `BOB_REFLY=N` chains missions; two consecutive fly→debrief→fly→debrief cycles run in one process (both debriefs render; 2nd `StartFlying` clean — DD7 fix holds across cycles); per-mission pre-flight + per-flight auto-quit re-arm. The game's own **menu→mission→fly→debrief→next** loop runs end-to-end. R2.3 + env-var-free packaging → Sprint 7. |
| 7+ | ~31 | — | — | R2.3 uninit-state grind (deeper mission variety) + "no env vars" packaging (drive the loop from real menu clicks; retire `BOB_STARTFLYING`/`BOB_AUTOQUIT`); then R3 polish/peripherals (joystick, save/load, Smacker, blit subsystem, render fidelity). |
