...
| R4.3 | **Campaign mission flow** — ◐ **A CAMPAIGN MISSION FLIES (2026-06-24).** Full loop runs: strategic map → **intercept a raid** (`BOB_CAMPAIGN_FLY` performs `OnClickedRbuttonauthorise` — `NewPackage(live-raid-squadron, RAF intercept profile)` scrambles a real interceptor) → **mission briefing** (`bobfrag` renders) → **Fly** → **campaign mission flight** (cockpit on real GL `:0`). Fly seam = the **QM flight path**: `bobfrag`'s Fly item → `quickmissionflight` → `StartFlying` → the always-on Launch3d bridge fires → `InThe3D=1` (frame 120 dumped, 800×600, glErr=0). One fix: a just-scrambled interceptor is `PS_ACTIVE_MIN`, outside the briefing's flyable-status window, so `SetPlayersPositionCamp` left `playersquadron=-1` and `FragFly2` blocked Fly → scaffold sets `MMC.playersquadron` from the package at Fly time. Cockpit renders (gunsight/instruments/Ammo 2800; scrambled sqn starts on the runway, 5ft/0Kts — faithful). Key structural finding: **campaign flight reuses the QM Launch3d bridge** (not a new subsystem). Remaining: in-cockpit Continue/Quit campaign dialog, debrief→next-day (reuses R2.2), briefing widget population (R6.3). _(earlier same day: briefing reached + the interception scaffold; SPIKED clock + MoveAllSAGs crash fixed in R4.5.)_ | 13 | ◐ |
| R4.4 | **Campaign save/load** — ◐ **LOAD SCREEN RENDERS; SAVE GATED ON CAMPAIGN (2026-06-23).** The `loadgame` (`CLoad`) screen reaches + renders (RAF/LW/Back/Load + hosted file-list); `CFiling::SaveGame` serialises `Miss_Man`+map view, so saves are produced only by campaign play → save/load is an R4.3 dependency. BoB shares the `CLoad` class with MiG Alley, whose file-list render + click→`DoLoadGame` fixes (MA S12–S14) port near-directly once saves exist. | 8 | ◐ |
