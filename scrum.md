...
| R6.2 | **`IDirectPlay4` over sockets in `SRC/compat/`** — the object and `EnumConnections` are done (R6.1); this is `InitializeConnection`, `Open`/`EnumSessions` (host + join), `CreatePlayer`/`DestroyPlayer`, `Send`/`Receive`, `GetMessageCount`. **Every one of the 49 unimplemented methods already logs itself under `BOB_TRACE_DPLAY=1`**, so the next stopping point names itself instead of being hunted. Ends with two processes on localhost exchanging one packet. | 13 | ☐ |
